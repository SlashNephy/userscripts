...
const headers: {
  ...
  },
  {
    ...
    render: (b) => {
      ...
      return b.homepage ?? `https://github.com/SlashNephy/userscripts/blob/master/src/${b.id}.ts`
    },
  },
  {
    key: 'homepageURL',
  ...
  },
  {
    ...
    render: (b, mode) => {
      switch (mode) {
        case 'dist':
          return b.updateUrl ?? `https://github.com/SlashNephy/userscripts/raw/master/dist/${b.id}.user.js`
        case 'dev-chrome':
  ...
  },
  {
    ...
    render: (b, mode) => {
      switch (mode) {
        case 'dist':
          return b.downloadUrl ?? `https://github.com/SlashNephy/userscripts/raw/master/dist/${b.id}.user.js`
        case 'dev-chrome':
          return b.private === true
            ? `file://${join(process.cwd(), 'dist', 'private', `${b.id}.dev.user.js`)}`
            : `file://${join(process.cwd(), 'dist', `${b.id}.dev.user.js`)}`
  ...
  },
  {
    ...
    render: (b) => {
      ...
      return b.supportUrl ?? 'https://github.com/SlashNephy/userscripts/issues'
    },
  },
  {
    key: 'include',
